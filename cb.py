...
@dp.message(CommandStart())
async def start_command(message: Message, state: FSMContext):
    await message.answer("Кайф чел, ток мне нужен твой токен, можешь кинуть по фасту?")
...
async def process_token(message: Message, state: FSMContext):
    ...
    if len(token) >= 20:  # Rough validation for a token format
        # Assuming the token is stored for further use
        global crypto_pay
        try:
            crypto_pay = CryptoPay(token=token)
            await message.answer("Отлично! Теперь выбери, что сделать:", reply_markup=main_keyboard())
            await state.clear()
        except APIError as e:
